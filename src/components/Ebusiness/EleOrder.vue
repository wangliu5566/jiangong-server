...
export default {
  data() {
    return {
      ...
      columnsDetailList: [{
        ...
        render: (h, params) => {
          return h('div', [
            h("span", params.row.Price ? '¥' + params.row.Price : "¥0.00")
          ])
        }
      }, {
        title: '总价',
        ...
        render: (h, params) => {
          return h('div', [
            h("span", params.row.TotalMoney ? '¥' + params.row.TotalMoney : "¥0.00")
          ])
        }
      }],
      columnsList: [{
        ...
        render: (h, params) => {
          return h('div', [
            h("span", params.row.TotalMoney ? '¥' + this.formatPrice(params.row.TotalMoney, 2) : "¥0.00")
          ])
        }
      }, {
        title: '支付金额',
        ...
        render: (h, params) => {
          return h('div', [
            h('Button', {
              ...
              on: {
                click: () => {
                  this.orderObj = this.dataList[params.index]
                  this.showDetailModal = true;
                  this.detailList = this.dataList[params.index].OrderDetails
                }
              }
            }, '查看订单'),
          ]);
  ...
  methods: {
    ...
    getlist() {
      ...
      this.$http.get(this.ApiTransfer("/Order/List"), {
          params: this.ApiTransfer("/Order/List", {
            mediaType: 'Elec', //书籍类型：1是电子书，2是纸质书，3是POD
