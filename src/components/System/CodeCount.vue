  ...
  <div class="statistic-col">
    <Row type="flex" justify="space-between">
      <div class="count-class ">
        <div class="spanText">
          <span class="countTitle">机构验证码数量统计<span class="count-sub-title">/总数量：{{total}}</span></span>
        </div>
...
export default {
  ...
  methods: {
    ...
    getListData() {
      ...
      myChart.showLoading();
      this.$http.get(this.ApiTransfer("/ValidateCode/ValidateCodeStatisticsData"), {
          params: this.ApiTransfer("/ValidateCode/ValidateCodeStatisticsData", paramsData)
        })
        .then((res) => {
          if (res.data.Success) {
            myChart.hideLoading();
