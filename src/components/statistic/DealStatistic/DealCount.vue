  ...
  <div class="statistic-col">
    <Row type="flex" justify="space-between">
      <div class="countClass ">
        <div class="spanText">
          <span class="countTitle">订单数统计<span class="count-sub-title">/总数量：{{total}}</span></span>
        </div>
...
export default {
  ...
  methods: {
    ...
    getListData() {
      ...
      myChart.showLoading();
      this.$http.get(this.ApiTransfer("/Report/OrderCountStatisticsByDate"), {
          params: this.ApiTransfer("/Report/OrderCountStatisticsByDate", paramsData)
        })
        .then((res) => {
          if (res.data.Success) {
            myChart.hideLoading();
