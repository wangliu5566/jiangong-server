  ...
  <div class="statistic-col">
    <Row type="flex" justify="space-between">
      <div class="countClass ">
        <div class="spanText">
          <span class="countTitle">知识元阅读量统计<span class="count-sub-title">/总数量：{{total}}</span></span>
        </div>
...
export default {
  ...
  methods: {
    ...
    getListData() {
      ...
      myChart.showLoading();
      this.$http.get(this.ApiTransfer("/Report/WordCountStatisticsByDate"), {
          params: this.ApiTransfer("/Report/WordCountStatisticsByDate", paramsData)
        })
        .then((res) => {
          if (res.data.Success) {
            myChart.hideLoading();
